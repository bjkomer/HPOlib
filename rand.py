...
def build_rand_call(config, options, optimizer_dir):
    #For Rand we have to cd to the exp_dir
    call = "cd " + optimizer_dir + "\n" + \
           "python " + os.path.dirname(os.path.realpath(__file__)) + \
           "/randcall.py"
    call = ' '.join([call, '-p', config.get('RAND', 'space'),
                     "-a", config.get('DEFAULT', 'algorithm'),
                     "-m", config.get('RAND', 'numberEvals'),
                     "-s", str(options.seed)])
    if options.restore:
        call = ' '.join([call, '-r'])
    return call
...
def restore(config, optimizer_dir, **kwargs):
    ...
    rand_trials = state['trials']._trials
...
def main(config, options, experiment_dir, **kwargs):
    ...
    if options.restore:
        ...
        optimizer_dir = os.path.join(experiment_dir, "rand_" +
                                     str(options.seed) + "_" +
                                     time_string)

    # Build call
    cmd += build_rand_call(config, options, optimizer_dir)

    # Set up experiment directory
    if not os.path.exists(optimizer_dir):
        os.mkdir(optimizer_dir)
        space = config.get('RAND', 'space')
        # Copy the hyperopt search space
        if not os.path.exists(os.path.join(optimizer_dir, space)):
            os.symlink(os.path.join(experiment_dir, "rand", space),
