...
def build_anneal_call(config, options, optimizer_dir):
    #For ANNEAL we have to cd to the exp_dir
    call = "cd " + optimizer_dir + "\n" + \
           "python " + os.path.dirname(os.path.realpath(__file__)) + \
           "/annealcall.py"
    call = ' '.join([call, '-p', config.get('ANNEAL', 'space'),
                     "-a", config.get('DEFAULT', 'algorithm'),
                     "-m", config.get('ANNEAL', 'numberEvals'),
                     "-s", str(options.seed)])
    if options.restore:
        call = ' '.join([call, '-r'])
    return call
...
def restore(config, optimizer_dir, **kwargs):
    ...
    anneal_trials = state['trials']._trials
    for trial in anneal_trials:
