...
version_info = ("# %76s #\n" % "https://github.com/hyperopt/hyperopt/gp_tree/486aebec8a4170e4781d99bbd6cca09123b12717")
__authors__ = ["Katharina Eggensperger", "Matthias Feurer"]
__contact__ = "automl.org"


def build_gp_tree_call(config, options, optimizer_dir):
    #For Tree we have to cd to the exp_dir
    call = "cd " + optimizer_dir + "\n" + \
           "python " + os.path.dirname(os.path.realpath(__file__)) + \
           "/gp_treecall.py"
    call = ' '.join([call, '-p', config.get('GP_TREE', 'space'),
                     "-a", config.get('DEFAULT', 'algorithm'),
                     "-m", config.get('GP_TREE', 'numberEvals'),
                     "-s", str(options.seed)])
    if options.restore:
        call = ' '.join([call, '-r'])
    return call
...
def restore(config, optimizer_dir, **kwargs):
    ...
    gp_tree_trials = state['trials']._trials
    for trial in gp_tree_trials:
...
def main(config, options, experiment_dir, **kwargs):
    ...
    if options.restore:
        ...
        optimizer_dir = os.path.join(experiment_dir, "gp_tree_" +
                                     str(options.seed) + "_" +
                                     time_string)

    # Build call
    cmd += build_gp_tree_call(config, options, optimizer_dir)

    # Set up experiment directory
    if not os.path.exists(optimizer_dir):
        os.mkdir(optimizer_dir)
        space = config.get('GP_TREE', 'space')
        # Copy the hyperopt search space
        if not os.path.exists(os.path.join(optimizer_dir, space)):
            os.symlink(os.path.join(experiment_dir, "gp_tree", space),
