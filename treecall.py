...
def main():
    ...
    if options.random:
        ...
        suggest = hyperopt.tree.rand.suggest
    else:
        suggest = hypertree.tree.suggest
    ...
    if options.restore:
        ...
        cPickle.dump({"trials": trials, "domain": domain, "rstate": rstate}, fh)
        fh.close()

    for i in range(int(options.maxEvals) + 1):
        # in exhaust, the number of evaluations is max_evals - num_done
        rval = hyperopt.FMinIter(suggest, domain, trials, max_evals=i,
                                 rstate=rstate)
        rval.exhaust()
        fh = open(state_filename, "w")
        cPickle.dump({"trials": trials, "domain": domain, "rstate": rstate}, fh)
